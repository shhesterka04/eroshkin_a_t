...
public:
	...
	public:
		...
		operator bool() const;
	private:
		T& hold;//ссылка на инт в котором будет мен€тьс€ бит
		T bit_position;//позици€ внутри  hold
	};
...
}; // ѕри инициализации битсета по копии мы перенимаем 2 основных параметра: размер и хранимое битсета
	...
	datasize(0)
{
	...
	if (filler == true)
		fill(data.begin(), data.end(), ~T(0)); //≈сли флаг равен 1, то заполнить массив -1 почему -1
	datasize = size;
};
...
typename BitsetT<T>::BitHolder BitsetT<T>::operator[](const T position) {
	...
	return BitHolder(position, (*this));// ≈сли все ок, возращает битхолдер
}
...
{}; // »нициализаци€ битхолдера: присваиваем все, кратное биту
...
BitsetT<T>& BitsetT<T>::operator>>=(const int64_t rhs) {
	...
	for (int i = datasize - rhs; i < datasize; i++)
		(*this)[i] = 0;
	return *this;
}; //“ак как индексаци€ идет от старшего разр€да, крутим цикл до начала вектора, выполне€ смещение значений на rhs. ќстатки
//от смешени€ занул€ем. 
...
}; //—охран€ем старый размер вектора, и создаем новый размер вектора. –асшир€ем вектор и вносим туда данные.
...
std::ostream& operator<<(std::ostream & ostream, const BitsetT<T>&data) {
	for (int i = data.size() - 1; i >= 0; i--)
		ostream << data[i];
	return ostream; // ¬ыводим каждый битик
};
